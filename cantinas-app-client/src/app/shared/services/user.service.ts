...
export class UserService {
    ...
    public getAllUsers(): Observable<User[]> {
        return this.http.get<User[]>('app/api/v1/users').pipe(
    ...
    public getUserById(userId: string): Observable<User> {
        return this.http.get<User>(`app/api/v1/users/${userId}`).pipe(
            retry(3), // retry a failed request up to 3 times
            catchError(this.handleError) // then handle the error
        );
    }
    ...
    public createUser(user: User): Observable<User> {
        return this.http.post<User>('app/api/v1/users', user).pipe(
    ...
    public deleteUserById(userId: string): Observable<{}> {
        const url = `app/api/v1/users/${userId}`;
        return (
            this.http
                // .delete(API_URL + '/users/' + userId)
